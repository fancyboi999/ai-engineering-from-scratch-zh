...
> 工具的安全性不取决于 Claude 是否谨慎使用，而取决于系统能否拒绝未授权的使用。
...
结构性修复其实更简单：不要暴露角色不需要的能力，传递调用方身份，并在工具执行时强制检查
权限范围和审批状态。
...
MCP 不会取代所有 API。对于稳定的内部服务调用，直接 API 可能更清晰、更快。当多个 host 需要
用统一方式发现和调用能力，或工具所有权应该留在 server 边界之后时，MCP 才真正有价值。
...
使用权限路径探索器，跟踪身份如何从已验证身份的请求开始，依次经过能力发现、模型选择、执行时
...
[`outputs/least-privilege-review.json`](../outputs/least-privilege-review.json) 是填写完成的能力审查，
其中展示了可见工具，以及一次被拒绝退款尝试的结构化结果。
...
| 最小权限 | 增加确认 | 移除不必要的能力，并尽可能缩小每一道剩余权限边界 |
