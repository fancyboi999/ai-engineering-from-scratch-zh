...
> 架构交付不以画完图为终点。下一位负责人能够接手并落实这项决策，才算真正交付。
...
明确表达置信程度。与其把未经测试的延迟或成本数字当成事实，不如坦诚说明“我们估算”。
...
建议变更的人，应该与有权接受其风险的人分开。

### 设计移交验收

只有接收团队能安全地运维和变更系统，移交才算完成；发出文档链接不算。
...
衡量采用情况时要结合结果和质量，不能只看登录量。高使用量可能来自强制流程或反复返工。
...
| 移交 | 发送文档 | 带证据转移运维能力和已接受的责任 |
| 运维就绪 | 部署成功 | 负责人、控制、可观测性、恢复、评估和支持均经过验证 |
| 采用 | 用户数 | 能够产生预期结果且没有隐藏负担的持续工作流使用 |
