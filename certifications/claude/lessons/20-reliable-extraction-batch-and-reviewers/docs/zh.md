...
> 有效 JSON 只证明形状保留下来，不能证明事实保留下来。

**类型：** Reference
**语言：** Python
**前置要求：** [验证主张，而不是置信度](../../05-output-evaluation-and-validation/)、[结构化输出是不可信合约](../../09-structured-output-and-defensive-parsing/)；阶段 14 第 39 课
**预计时间：** 约 135 分钟
...
每个示例都应展示理由，而不只是答案。不要用重复的简单案例淹没上下文。
...
必填加 nullable 会强制明确决定：有证据的值，或已知缺失。它避免字段被静默省略。
...
不要只说“再试一次”。保留原始来源和先前结果，限制重试。反复语义失败应升级处理，而非把不确定性变成延迟和成本。
...
用人工标签衡量 reviewer 的 precision 和 recall。模型 judge 是测量仪器，不是 ground truth。
...
将一个有证据支持的日期改成编造值，运行四层验证，并将失败记录路由到 adjudication，而不是再进行盲目重试。
