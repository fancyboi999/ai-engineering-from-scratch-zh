...
专业的架构设计始于发现。考试会检验你能否把业务意图转化为解决方案，并为其中的取舍
给出站得住脚的理由。在实际工作中，这项能力能避免数月返工。
...
它会展示一个能力足够的系统何时仍会带来负面的工作流价值，或违反不可让步的门槛。
...
根据需求为每个方案评分。能力最强的方案不一定最好。如果工作流稳定且路径已知，确定性编排
...
如果答案没有厘清权限和错误成本，就立即选择最大模型、添加 agent 或承诺自动化，应保持警惕。
