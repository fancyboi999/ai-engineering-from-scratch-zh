...
- 在聊天、Projects、Research、文件与 Artifacts、connectors 和编程载体之间做出选择。
...
输出很漂亮，却引用了上次对话里的旧产品价格，遗漏了内部文档中的政策变更，还包含一条没有来源的竞品主张。问题并非始于措辞，而是始于所选的工作载体。

旧聊天带来了过时的上下文。粘贴链接不能保证研究足够全面。内部政策不在可用知识范围内。工作流也没有主张核验步骤。

考试目标叫作“产品与模型选择”，真正考验的却是边界设计。你要决定 Claude 能看到什么、记住什么、检索什么、创建什么，以及这项任务值得投入多少推理能力。
...
它的优势不只是存储，而是可重复性。每段新对话都从一条刻意设定的边界开始。
...
Cowork 是面向多步骤知识工作的产品载体，不是独立部署路径，也不是本课的考试目标。截至 2026 年 8 月 9 日，Anthropic 当前帮助资料将它描述为目标驱动的任务循环：你描述结果、审查方案、观察进度，并在运行过程中引导或调整工作方向。Projects 可以为相关任务提供长期文件、链接、指令和记忆。Skills 提供可复用工作流，plugins 则可封装 skills、connectors、agents 和 hooks。
...
Research 不会替你判断来源。长篇报告仍可能引用薄弱证据、混用不同日期的主张，或漏掉私有约束。把引用当作通往证据的导航，而不是自动成立的证明。
...
### Connectors 用实时、受权限控制的访问替代复制

Connectors 让 Claude 从外部服务检索内容或在其中执行操作。当来源新鲜度很重要，而手动复制粘贴会逐渐偏离时，它们很有用。

不要因为存在某个 connector 就选择它。先检查：
...
- 哪些数据会随对话一并保留。
- 是否需要组织管理员启用。
- connector 是否能提供你所需的确切内容类型。

截至 2026 年 8 月 8 日，官方文档称 Google Workspace connectors 可以搜索 Gmail、使用 Calendar 和 Drive，执行操作时需要明确批准。文档也列出了部分内容可能不可见等限制。这些都属于可能变化的产品事实。
...
这些行不是排名，而是责任归属图。最佳路径，是以最少的新控制平面满足组织约束的路径。
...
3. 使用 Research 广泛收集公开资料，并使用获批 connector 或维护中的 Project 来源获取内部文档。
...
使用模型适配图调整重复性、新鲜度、后果、协作和输出约束。重点不是寻找一个永远最好的载体，而是观察哪项约束会让更简单的载体不再适用。
...
运行本地适配评分器，然后让较便宜的模型无法通过某个门槛，或让更简单的载体满足全部约束。改变部署权重、破坏候选分数，或删除带日期的证据。推荐结果必须随证据变化，而不是随产品或云偏好变化。
...
- 最新外部数据或外部操作适合获批 connector 或自有集成。
...
- 选择实测质量达标的最小模型，而不是名气最大的模型。
- 旧上下文更可能造成干扰而不是帮助时，重新开始。
...
- [用 connectors 扩展 Claude](https://support.claude.com/en/articles/11176164-use-connectors-to-extend-claude-s-capabilities)
- [使用 Google Workspace connectors](https://support.claude.com/en/articles/10166901-use-google-workspace-connectors)
