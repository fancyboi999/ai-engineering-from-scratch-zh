...
一名架构师为六个预期用例准备了六张图。支持图使用 agent 循环，代码图使用 Claude Code，研究图有 subagent，提取图使用 JSON。
...
本课程不复现考试场景。你将创建名为 Cedar Bridge 的原创系统，它是一家虚构的软件和服务公司。每个透镜都强调同一架构的不同部分。
...
| 提取 | 将变更通知标准化为记录 | schema 有效但值系编造或无证据支持 |

你不需要六套不相关的平台。你需要一个核心架构和明确的变化点。
...
定义任务、前置条件、上下文边界、允许工具、完整或部分状态以及合并规则。固定顺序使用代码，语义选择使用模型推理。subagent 必须有理由：隔离、专门化、独立审核或安全并行。
...
每个工具需要一个动作和对象、正反两面的选择指引、闭合 schema、权限范围、副作用声明和结构化错误契约。写工具需要新鲜授权、幂等性和对账。
...
项目指引应简洁、带版本并共享。把文件特定要求放在路径规则中，将可复用方法打包为 Skill，将明确用户工作流打包为命令。使用 hook 强制确定性的范围与命令控制。
...
使用就绪矩阵在六种场景透镜中测试全部五道架构门禁。改变工具、配置、验证或上下文不变量，观察哪些场景会被阻塞，而不是依赖一种拓扑。
...
架构包和填写完成的 [`outputs/demo-readiness-report.json`](../outputs/demo-readiness-report.json) 是实用产物。
...
使用随附 Python 校验器展示的形状。校验器有意检查架构不变量，而不是文案质量。
...
它无法证明模型总会正确选择、政策有效，或人工负责人合格。请增加场景评估和组织审核。
...
每项包含背景、选择、被否决备选、后果、证据、变更触发条件和负责人。ADR 不是产品偏好，而是解释此选择为何适合该场景。

### 第 7 步：进行独立审核

给一名全新审核人提供架构包、校验器输出、威胁样本和评分规则。不要提供有说服力的设计记录。要求对方给出带稳定 ID、受影响领域、证据、严重级别和所需修复的发现。
...
- **把恢复当作恢复：** 过期对话取代外部状态对账。
- **每个场景名称一套设计：** 共享架构原则从不迁移。
...
- **架构移交：** 为安全实施所需的决策、证据、控制、缺口和下一责任方组成的包。
