...
- 区分聊天上下文、Project 指令、Project 知识、记忆、connectors、检索和 API prompt 缓存。
...
三个月后，Claude 根据旧路线图推荐了发布日期。这个日期存在于 Project 知识中，也出现在多份历史会议记录里，却与 connector 中保存的较新决策冲突。由于上下文反复提供错误答案的证据，回复听起来非常确定。

团队把它称作幻觉。其实这主要是知识管理失败：他们把 Project 当成文档仓库，把记忆当成权威来源，还把检索当成事实保证。

上下文更多，不等于上下文更好。可信系统知道哪些事实只是临时信息、哪些具有权威性，以及谁负责让它们保持最新。
...
| Connectors | 在当前权限下访问外部系统 | 误解来源权限、同步状态或新鲜度 |
| 检索 | 从大型语料库中选择相关片段 | 看似相关的文本不完整或权威性低 |
| Prompt 缓存 | 高效复用稳定的 API prompt 前缀 | 缓存动态内容或忽略失效机制 |

一项功能可以承担多种职责，但区分职责能避免类别错误。记忆可以提醒 Claude 你偏好简洁报告，却不应悄悄成为当前退款政策的来源。Connector 可以暴露最新文件，却不能证明该文件已经获批。

### 上下文有注意力预算

更大的上下文窗口提高的是容量，不是确定性。每增加一份文档，都会加剧注意力竞争，也多一次产生矛盾的机会。
...
- [Anthropic 帮助中心：用 connectors 扩展 Claude 的能力](https://support.claude.com/en/articles/11176164-use-connectors-to-extend-claude-s-capabilities)
...
Projects、记忆、connectors、检索模式和 prompt 缓存的名称、可用性、限制、保留行为与价格都可能变化。这些来源于 2026-08-08 核验。部署或备考前，请核对当前官方产品与隐私文档。
