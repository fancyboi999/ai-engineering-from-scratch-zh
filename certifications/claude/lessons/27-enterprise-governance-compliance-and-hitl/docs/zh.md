...
正确的回应也不是直接否决，而是设计一套受治理的系统：建立数据地图，作出风险决策，为控制
...
不同产品功能可能有不同的数据保留和适用资格规则。文件、批处理、代码执行、MCP connector、
托管会话和标准 Messages 请求的边界可能各不相同。针对确切的功能组合，检查当前官方文档和
你方签订的合同。

### 先最小化，再保护

不必要的数据根本不进入系统，安全控制才更有力。
...
只有审核人有能力改善决策，并且具备所需的信息、时间、专业能力和权限时，人在环才有价值。
...
使用审批流程探索器调整后果、可逆性、证据、审核人资质、队列容量和回退方案。它会清楚展示
“人工审核”何时是真正的控制，何时只是瓶颈。
...
登记册、数据边界、预防性、检测性、纠正性和治理性控制，以及人员配置完备的审批路径。
...
对于患者消息工作流，更安全的首个版本可以只为合格审核人起草路由建议，不发送回复，也不修改
医疗记录。它只使用最低限度的必要字段和可信临床来源，严格执行租户和角色授权，输出链接到
来源，检查高风险关键词和证据，并在审核队列不可用时安全回退。
...
1. 为一个使用文件、MCP connector、批量评估和人工审核的客服工作流构建数据地图。
