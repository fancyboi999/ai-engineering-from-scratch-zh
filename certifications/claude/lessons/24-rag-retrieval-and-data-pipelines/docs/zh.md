...
这是检索事故。把它当成模型事故处理，不仅浪费时间，还可能掩盖真正失效的控制措施。
...
修改代码之前，先在排序实验中比较词法匹配、元数据过滤、过期来源排除和 top-K 行为。可见的
排名把一次检索决策与召回率、倒数排名、新鲜度和溯源信息联系起来。
...
实验使用 Python 标准库实现了一个小型 BM25 风格索引，特意保持透明。生产搜索系统速度更快、
能力更强，但评分和元数据边界不应该再显得神秘。
...
索引会计算词频、文档频率、长度归一化和逆文档频率分数。精确的查询词可以提高真正包含当前
有效政策用语的来源排名。
