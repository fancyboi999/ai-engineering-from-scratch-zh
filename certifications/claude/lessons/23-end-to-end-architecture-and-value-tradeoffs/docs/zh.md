...
系统失败，不是因为 prompt 还缺一句话，而是因为几种不同的职责被压进了一个概率性步骤。
...
反馈不是一堆点赞事件。它必须把结果与输入、版本、执行轨迹和决策联系起来。
...
明确哪些证据会促使团队切换模式。架构是一项当前决策，不是永久身份。
...
现在再加入一小时的答复时限和严格的成本预算。最优设计可能随之改变。架构始终是在约束下
做架构。
...
生产质量体现在含义模糊、数据过期、权限错误、超时和部分失败等场景中。批准架构之前必须把
这些场景纳入考虑。
