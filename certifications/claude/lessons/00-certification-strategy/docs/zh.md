...
对 Associate Foundations 而言，权重最大的领域是输出评估与验证。这是一个信号：该角色不只是会向 Claude 提问的人，而是能判断答案是否适合使用的人。
...
反例至关重要。若你说不出某条规则何时不适用，说明你记住的可能只是口号，而不是边界。
...
使用路线图图表调整领域信心和可用时间。观察薄弱且权重高的领域如何改变学习顺序，而不是把每个目标等量对待。
