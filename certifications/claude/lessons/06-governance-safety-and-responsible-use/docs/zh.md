...
**类型：** Learn
**语言：** Python
**前置要求：** [把每项事实放进正确的上下文](../../04-context-knowledge-memory-and-caching/)、[验证主张，不看自信](../../05-output-evaluation-and-validation/)、[护栏](../../../../../phases/11-llm-engineering/12-guardrails/)
**预计时间：** 约 110 分钟
...
如果任何答案未知，正确的下一步可能是澄清政策，而不是开始生成。

### 能力、许可与决策权相互独立

Claude 在技术上可以起草优惠方案。Connector 可能有权限访问客户记录。两者都不代表工作流获准批准折扣或发送消息。
...
假设来源内容可能包含恶意指令。把检索到的文档当作数据，不是命令。清楚划定边界，把工具授权放在模型文本之外。
...
使用置信度—风险图调整证据置信度、后果、可逆性和受影响群体。该交互会展示：即使输出置信度很高，只要决策权或影响程度较高，仍然需要审核。
...
- **把技术访问当决策权：** 把 connector 权限误认为业务批准。
...
4. 按最小权限原则重新设计一项宽泛的 connector 权限。
5. 为一条后果严重的建议编写批准数据包。
