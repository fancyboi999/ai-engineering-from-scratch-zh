...
仪表盘衡量的是传输成功，产品依赖的是任务成功。可观测性必须把两者连接起来。
...
trace 连接完整执行轨迹。一条 trace 应通过父子时间关系展示模型调用、检索、工具执行、验证、
...
为每个主要 span 跟踪 P50 和 P95。P50 描述普通路径，P95 暴露缓慢工具、长上下文、速率限制和
重试。
...
每种手段都可能改变质量或安全性。应在有代表性的评估集上衡量取舍。
...
离线评估必不可少，但还不够。生产流量会带来新的查询、数据、负载和集成。
...
使用 SLO 探索器分别调整任务成功率、缓存率、重试成本、P50 和 P95。它会揭示这样的变体：调用
更便宜或传输状态健康，却仍然无法通过用户体验、质量或每次成功成本门禁。
...
`by_variant` 防止采用缓存或路由的设计被平均进基线。把质量、延迟和成本放在一起比较。
