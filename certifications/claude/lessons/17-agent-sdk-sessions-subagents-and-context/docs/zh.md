...
**类型：** Reference
**语言：** Python
**前置要求：** [Agent SDK 是运行框架，不是权限](../../12-claude-agent-sdk-and-hooks/)、[多 Agent 编排与委派](../../16-multi-agent-orchestration-and-delegation/)；阶段 14 第 17 课
**预计时间：** 约 120 分钟
...
压缩能节省上下文，但不会形成持久执行记录、保证关键事实留存，或验证信息的新鲜度。
...
环境发生实质偏离时，应以新计划启动或 fork，而不是强迫旧 session 重新解释自身。
...
连续性仍然有效时选择恢复；需要隔离替代方案时选择 fork；陈旧上下文构成风险时选择新 session。压缩解决的是大小，不是真实性。
