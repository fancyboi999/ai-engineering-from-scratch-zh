...
> Claude 完成任务，不代表工作流已经完成。只有下一个人能验证、决策、行动并恢复，工作流才算完整。

**类型：** Learn
**语言：** Python
**前置要求：** [验证主张，不看自信](../../05-output-evaluation-and-validation/)、[让能力受权责边界约束](../../06-governance-safety-and-responsible-use/)、[Anthropic 工作流模式](../../../../../phases/14-agent-engineering/12-anthropic-workflow-patterns/)
**预计时间：** 约 105 分钟
...
成功的工作流不是一串模型调用，而是一串职责，带有明确证据和可恢复状态。
...
3. **重新设计：** 当前步骤只是输入糟糕或系统重复造成的浪费，应删除或重构。
...
接手人必须知道还有哪些职责归自己。“请审核”并不完整。“确认 R-14 和 R-19 已获准对外发布；其余检查均已通过”才可以直接行动。

### 在失败时保留状态

长工作流会失败。API 会超时，connectors 会返回部分数据，审核者会错过截止时间，输入也可能在生成后发生变化。
...
Claude 产品功能、connector 行为、模型能力、限制和成本都可能变化。这些来源于 2026-08-08 核验。把工作流从影子模式转入生产前，应重新核验当前官方文档和组织控制。
