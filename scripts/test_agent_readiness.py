...
SITE = ROOT / "site"
WEBSITE_ID = "https://aieng-zh.cn/#website"
COURSE_ID = "https://aieng-zh.cn/#course"
...
def main() -> None:
    ...
    # 中文站保留 lesson.html / certification.html 作为真实兼容入口，不把它们
    # 重定向到上游 extensionless canonical；主课程 canonical 始终是 /lessons/**。
    legacy_routes = {route["src"]: route for route in config.get("routes", [])}
    assert r"/lesson\.html" not in legacy_routes
    assert r"/certification\.html" not in legacy_routes
    ...
    assert openapi["servers"][0]["url"] == "https://aieng-zh.cn"
    paths = openapi["paths"]
    assert {
        "/llms.txt",
        "/sitemap.xml",
    ...
    core_lessons = {
        lesson_path: entry
        for lesson_path, entry in lessons.items()
        if lesson_path.startswith("phases/")
    }
    certification_lessons = {
        lesson_path: entry
        for lesson_path, entry in lessons.items()
        if lesson_path.startswith("certifications/")
    }
    assert all(
        entry["canonicalUrl"]
        == "https://aieng-zh.cn/lessons/" + lesson_path.removeprefix("phases/") + "/"
    ...
    assert all(
        entry["canonicalUrl"].startswith(
            "https://aieng-zh.cn/certification?id="
        )
    ...
    assert sitemap.count("/lessons/") == len(core_lessons)
    sitemap_lessons = {
        "phases/" + value.strip("/")
    ...
    assert {"WebSite", "Course"} <= set(home_graph)
    ...
    assert "https://aieng-zh.cn" in identity_json
    assert "aiengineeringfromscratch.com" not in identity_json
