      ...
      manifest: {
        ...
        lessons: {
          'phases/01-math/01-vectors': {
            ...
            sourceUrl: 'https://github.com/fancyboi999/ai-engineering-from-scratch-zh/tree/main/phases/01-math/01-vectors',
            canonicalUrl: 'https://aieng-zh.cn/lessons/01-math/01-vectors/',
          },
          'phases/07-transformers/09-vectors': {
            ...
            learningPathIds: [],
            fromTrackIds: [],
            canonicalUrl: 'https://aieng-zh.cn/lessons/07-transformers/09-vectors/',
          },
      ...
      manifest: {
        ...
        tracks: {
          'claude-example': {
            ...
            canonicalUrl: 'https://aieng-zh.cn/certification?id=claude-example',
            lessons: [
...
test('lesson route renders unique crawlable HTML with a path-only canonical', function () {
  ...
  assert.match(response.body, /rel="canonical" href="https:\/\/aieng-zh\.cn\/lessons\/01-math\/01-vectors\/"/);
...
test('certification route renders a crawlable track with an id-only canonical', function () {
  ...
  assert.equal(response.statusCode, 200);
  assert.match(response.headers['cache-control'], /s-maxage=86400/);
  assert.match(response.body, /rel="canonical" href="https:\/\/aieng-zh\.cn\/certification\?id=claude-example"/);
...
test('deployment routes extensionless pages through handlers and redirects legacy HTML URLs', function () {
  ...
  assert.match(lessonTemplate, /aieng-zh\.cn\/lesson\.html/);
  assert.doesNotMatch(certificationsScript, /aiengineeringfromscratch\.com/);
});

test('site runtime sources keep zh static lesson canonicals and contextual routes', function () {
  const repoRoot = path.join(__dirname, '..');
  [
    'site/index.html',
    'site/app.js',
    ...
    assert.doesNotMatch(source, /aiengineeringfromscratch\.com/, relativePath);
  });
});
...
test('server and browser source links honor generated repository identity', function () {
  ...
  assert.match(lessonTemplate, /AIFSContentSource/);
