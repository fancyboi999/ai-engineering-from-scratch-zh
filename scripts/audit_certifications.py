...
LEARNING_OBJECTIVES_RE = re.compile(r"^##\s+学习目标\s*$", re.MULTILINE)
...
# 中文技术文档可以正常使用破折号；是否滥用由翻译审校判断，不做字符级一刀切。
DISALLOWED_DASHES: dict[str, str] = {}
OFFICIAL_EXAM_FACTS = {
...
def load_json(audit: Audit, path: Path) -> Any | None:
    if path.is_symlink():
        audit.add("C001", path, "JSON source must be a regular file, not a symlink")
        return None
    if not path.is_file():
...
def check_answer_quality(audit: Audit, path: Path, questions: list[Any]) -> None:
    ...
    for question in questions:
        ...
        metrics = [("characters", [len(str(option)) for option in options])]
        # \w+ 会把整段连续中文当成一个“单词”，不是有意义的长度信号。
        if not any(re.search(r"[\u3400-\u9fff]", str(option)) for option in options):
            metrics.append(("words", [len(re.findall(r"\w+", str(option))) for option in options]))
        for metric, lengths in metrics:
        ...
        if len(correct) == 1:
            single_count += 1
            has_cjk = any(re.search(r"[\u3400-\u9fff]", str(option)) for option in options)
            lengths = [len(str(option)) for option in options] if has_cjk else [len(re.findall(r"\w+", str(option))) for option in options]
...
def check_code_and_tests(audit: Audit, lesson_dir: Path, doc_text: str) -> None:
    ...
    if declared is None:
        audit.add("C024", lesson_dir / "docs" / "zh.md", "missing **语言：** field")
    else:
        normalized = declared.strip().lower()
        if not expected and normalized not in {"none", "n/a", "-", "no code"}:
            audit.add("C025", lesson_dir / "docs" / "zh.md", f"语言字段声明了 {declared!r}，但没有 code/main.*")
        if expected:
            declared_set = {item.strip() for item in declared.split(",") if item.strip()}
            if declared_set != expected:
                audit.add("C025", lesson_dir / "docs" / "zh.md", f"语言 {sorted(declared_set)} 与 code main {sorted(expected)} 不一致")
    ...
    for extension in CODE_EXTENSIONS:
        ...
        if "docs/zh.md" not in source:
            audit.add("C026", main, "main 文件头部必须引用本课的 docs/zh.md")
...
def check_lesson(audit: Audit, lesson_dir: Path) -> None:
    ...
    doc_path = lesson_dir / "docs" / "zh.md"
    if doc_path.is_symlink():
        audit.add("C020", doc_path, "docs/zh.md must be a regular file, not a symlink")
        check_lesson_quiz(audit, lesson_dir)
        return
    if not doc_path.is_file():
        audit.add("C020", doc_path, "missing docs/zh.md")
        check_lesson_quiz(audit, lesson_dir)
        return
    text = doc_path.read_text(encoding="utf-8")
    prose_chars = len(re.sub(r"\s+", "", text))
    if prose_chars < 4000:
        audit.add("C020", doc_path, f"认证课程内容过薄：{prose_chars} 个非空白字符，最少 4000")
    if not H1_RE.search(text):
        audit.add("C020", doc_path, "missing top-level heading")
    if not LEARNING_OBJECTIVES_RE.search(text):
        audit.add("C020", doc_path, "missing 学习目标 section")
    ...
    if expected_figure and expected_figure not in figure_ids:
        audit.add("C029", doc_path, f"交互实验必须嵌入 figure {expected_figure!r}")
    ...
    for required in ("类型", "语言", "前置要求", "预计时间"):
...
def check_assessment_question(
    ...
    has_cjk_explanation = isinstance(explanation, str) and bool(re.search(r"[\u3400-\u9fff]", explanation))
    explanation_is_thin = (
        not isinstance(explanation, str)
        or (has_cjk_explanation and len(re.sub(r"\s+", "", explanation)) < 40)
        or (not has_cjk_explanation and len(explanation.split()) < 20)
    )
    if explanation_is_thin:
...
def check_ai_native_learning_surface(audit: Audit, actual_lessons: set[str]) -> None:
    ...
    if wrapper != skill:
        audit.add("C080", CLAUDE_CERT_SKILL_PATH, "Claude Code 导师 Skill 必须与可移植 Skill 完全一致")
    if CLAUDE_CERT_SKILL_OPENAI_PATH.read_text(encoding="utf-8") != CERT_SKILL_OPENAI_PATH.read_text(encoding="utf-8"):
        audit.add("C080", CLAUDE_CERT_SKILL_OPENAI_PATH, "Claude Code 导师元数据必须与可移植 Skill 完全一致")
    ...
    for token in ("GETTING_STARTED.md", "../../skills/claude-certification/SKILL.md", "EPUB/PDF"):
    ...
    # 中文镜像不提交上游的 EPUB/PDF 构建器；因此认证内容天然不会进入图书流水线。
...
def run_audit() -> Audit:
    ...
    if "不隶属" not in disclaimer or "anthropic" not in disclaimer:
        audit.add("C007", PROGRAM_PATH, "disclaimer 必须声明课程不隶属于 Anthropic")
    for page in PUBLIC_CERT_PAGES:
        if not page.is_file() or "不隶属" not in page.read_text(encoding="utf-8").lower():
            audit.add("C007", page, "公开认证页面必须展示不隶属声明")
