...
test('plain static preview rewrites every route-producing site surface', () => {
  ...
  const learningPathsSource = fs.readFileSync(path.join(__dirname, 'learning-paths.html'), 'utf8');
  const canonicalPathLinks = Array.from(
    learningPathsSource.matchAll(/href="(\/lessons\/[^"]+\?learningPath=[^"]+)"/g),
    match => match[1]
  );
  assert.ok(canonicalPathLinks.length > 0, 'learning paths must link directly to static lesson pages');
  assert.match(learningPathsSource, /class="career-guide-cta"/);
});
...
test('deployed contextual routes stay extensionless while zh canonicals keep static lesson URLs', () => {
  const production = loadRouteRuntime('https://aieng-zh.cn/index.html');
  assert.equal(
  ...
  assert.match(lesson, /<link rel="canonical" href="https:\/\/aieng-zh\.cn\/lesson\.html">/);
  assert.match(certification, /<link rel="canonical" href="https:\/\/aieng-zh\.cn\/certification\.html">/);
});
