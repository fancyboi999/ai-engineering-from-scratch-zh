  ...
  const context = {
    ...
    window: {
      ...
      location: {
        ...
        pathname: options.pathname || '/site/lesson.html',
        href: 'http://localhost' + (options.pathname || '/site/lesson.html'),
      },
    },
  };
  vm.runInNewContext(
...
test('shared site asset families use the expected cache keys on every page', () => {
  ...
  const navigationRelease = '20260831a';
...
test('build-time SEO manifests cover every readable lesson and expose canonical no-JavaScript discovery links', () => {
  ...
  const expectedCoursePaths = phases.flatMap(phase => phase.lessons.map(lesson => {
    ...
    if (!match || !fs.existsSync(path.join(root, match[1], 'docs', 'zh.md'))) return null;
    return match[1];
  }).filter(Boolean));
  ...
  assert.ok(visionTransformerEntries.every(Boolean));
  ...
  for (const track of trackEntries) {
    ...
    assert.match(track.canonicalUrl, /^https:\/\/aieng-zh\.cn\/certification\?id=/);
...
test('remote content source rejects dot-segment repositories and revisions', () => {
  ...
  assert.equal(
    invalid.rawRepoUrl('phases/00-setup-and-tooling/01-dev-environment/docs/zh.md'),
    'https://raw.githubusercontent.com/example-owner/ai-engineering-from-scratch-zh/preview/ref/phases/00-setup-and-tooling/01-dev-environment/docs/zh.md'
  );
  ...
  assert.throws(() => invalidFallback.rawRepoUrl('README.md'), /outside the allowed lesson trees/);
  assert.equal(invalidFallback.rawRepoUrl('phases/00-setup-and-tooling/01-dev-environment/docs/zh.md'),
    'https://raw.githubusercontent.com/example-owner/course/main/phases/00-setup-and-tooling/01-dev-environment/docs/zh.md');

  const valid = loadContentSource({
    source: { owner: 'example-owner', repo: 'course.repo', revision: 'feature/lesson-copy' },
  });
  assert.equal(valid.rawRepoUrl('phases/00-setup-and-tooling/01-dev-environment/docs/zh.md'),
    'https://raw.githubusercontent.com/example-owner/course.repo/feature/lesson-copy/phases/00-setup-and-tooling/01-dev-environment/docs/zh.md');

  const staticPreview = loadContentSource({
    pathname: '/lessons/00-setup-and-tooling/01-dev-environment/',
  });
  assert.equal(
    staticPreview.repoUrl('phases/00-setup-and-tooling/01-dev-environment/docs/zh.md'),
    'https://raw.githubusercontent.com/fancyboi999/ai-engineering-from-scratch-zh/main/phases/00-setup-and-tooling/01-dev-environment/docs/zh.md'
  );
});
...
test('repository Agent Skills path routes 22 to 24 and keeps 23 optional', () => {
  ...
  assert.equal(poisoningPreflight.title, '工具投毒与不可信指令');
...
test('optional MCP capstone keeps its prerequisite gate in every lesson reader surface', () => {
  ...
  assert.match(lessonHtml, /learningPathPrerequisiteCallout\(focusedEntry, '学习本课前需要完成'\)/);
  ...
  assert.match(lessonHtml, /focusedOptionalLocked \? '尚未解锁的选修综合项目：' : '选修综合项目：'/);
});
...
test('generic course skills dispatch every supported state to an installed owner', () => {
  ...
  for (const name of ['learn', 'start-learning']) {
    ...
    const section = source.match(/## 专门 Agent Skills 交接\s+([\s\S]*?)(?=\n## |$)/);
    ...
    assert.match(section[1], /(?:不要将 Agent Skills 状态复制到|不创建) `LEARNING\.md`/);
    const resume = source.match(/## 跨课程模式的续学路由\s+([\s\S]*?)(?=\n## |$)/);
    assert.ok(resume, `${name} is missing cross-route resume handling`);
    for (const [stateFile, owner] of routeOwners) {
      assert.match(
        resume[1],
        new RegExp('`' + stateFile.replace('.', '\\.') + '` 属于[^\n]*`' + owner + '`'),
    ...
    assert.match(resume[1], /续学请求中点名路线[\s\S]*?立即分派给(?:该)?所有者/);
    assert.match(resume[1], /收集存在状态文件的所有者/);
    assert.match(resume[1], /若(?:恰有|仅剩)一个路线所有者[\s\S]*?(?:恢复|调用)它/);
    assert.match(resume[1], /若有两个或更多(?:路线)?所有者/);
    assert.match(resume[1], /询问要恢复哪(?:条|一条)/);
    assert.match(source, /旧 runtime[\s\S]*?`learn-mcp-engineering`[\s\S]*?`learn-mcp`/);
    ...
    const genericStart = source.indexOf(name === 'learn' ? '## 第 0 步' : '## 第 1 步');
    assert.ok(source.indexOf('## 跨课程模式的续学路由') < genericStart);
  }
...
test('terminal quiz skills isolate answer keys and use neutral reply formats', () => {
  ...
  assert.match(placement, /请按此格式回复：Q1: <letter>, Q2: <letter>。/);
...
test('the vectors and matrices quiz has parallel choices and varied answer positions', () => {
  ...
  const expectedAnswers = [
    '第一个矩阵的列数等于第二个矩阵的行数',
    '与兼容矩阵相乘后保持该矩阵不变的方阵',
    '逐元素乘法按对应位置相乘；矩阵乘法计算行与列的点积',
    '在相加前把 b 扩展到兼容的 batch 维度',
    '矩阵奇异，不能有逆矩阵',
    '交换矩阵的行与列',
  ];
  ...
  assert.ok(new Set(questions.map(question => question.correct)).size >= 3);
  assert.match(questions[3].explanation, /b\[:, None\]/);
  assert.match(questions[3].explanation, /尾部维度对齐/);
  assert.match(questions[3].explanation, /不能可靠地沿 batch 轴扩展/);
  for (const question of questions) {
    const wordCounts = question.options.map(option => /[\u3400-\u9fff]/.test(option)
      ? Array.from(option.replace(/\s+/g, '')).length
      : option.trim().split(/\s+/).length);
...
test('course guide shape count matches its six routing bullets in both mirrors', () => {
  ...
  for (const file of [
    ...
    const source = fs.readFileSync(file, 'utf8');
    const routing = source.match(/1\. \*\*理解请求\*\*[\s\S]*?(?=\n2\. \*\*扫描 Contents 表格\*\*)/);
    assert.ok(routing, `${file} is missing the routing-shape section`);
    assert.match(routing[0], /六种形态/);
...
test('repository exposes the canonical Model Context Protocol learning path only', () => {
  ...
  assert.equal(modelContextProtocol.title, '模型上下文协议（MCP）');
...
test('repository exposes the four core AI engineering learning paths', () => {
  ...
  const domains = [
    ['building-and-deploying-ai-applications', '构建与部署 AI 应用', 12],
    ['software-engineering-fundamentals', '软件工程基础', 13],
    ['using-coding-agents', 'Agent 辅助工程', 16],
    ['shaping-the-build', '产品判断与交付', 8],
  ];
  ...
  const homepageTargets = new Map([
    ['building-and-deploying-ai-applications', ['构建与部署 AI 应用', 'building-and-deploying']],
    ['software-engineering-fundamentals', ['软件工程基础', 'software-fundamentals']],
    ['using-coding-agents', ['Agent 辅助工程', 'coding-agents']],
    ['shaping-the-build', ['产品判断与交付', 'shaping-the-build']],
  ]);
  for (const [id] of domains) {
    const [title, anchor] = homepageTargets.get(id);
    assert.match(
      homepage,
      new RegExp(`class="learning-paths-node [^"]+"[^>]+href="learning-paths\\.html#${anchor}"[^>]+aria-label="查看\\s*${title}所需的能力"`)
    );
  }
  assert.match(readme, /523 节课 · 20 个阶段/);
  assert.match(readme, /https:\/\/aieng-zh\.cn\/learning-paths\.html/);
  assert.doesNotMatch(readme, /<!-- STATS:START|<\/b> readers/);
...
test('homepage preserves live GitHub CTAs and the motion-aware learner marquee', () => {
  ...
  assert.match(mastheadCta[0], /<span>在 GitHub 上 Star<\/span>/);
  assert.match(homepage, /href="learning-paths\.html"/);
  assert.doesNotMatch(mastheadCta[0], /Start (?:MCP Engineering|Agent Skills)/i);
  assert.match(
    mastheadCta[0],
    /<a class="masthead-btn masthead-btn--primary" href="https:\/\/github\.com\/fancyboi999\/ai-engineering-from-scratch-zh"[^>]*>[\s\S]*?<span>在 GitHub 上 Star<\/span>[\s\S]*?<span class="masthead-btn-count" data-gh-stars="fancyboi999\/ai-engineering-from-scratch-zh" data-loading="true">/
  );
  assert.match(
    mastheadCta[0],
    /<a class="masthead-btn" href="https:\/\/github\.com\/fancyboi999"[^>]*>[\s\S]*?<span>关注 @fancyboi999<\/span>/
  );
...
test('homepage uses consistent responsive grids for controls, routes, and curriculum rows', () => {
  ...
  assert.match(
    homepage,
    /@media \(max-width: 600px\) \{[\s\S]*?\.course-route-actions\s*\{[\s\S]*?width: 100%;[\s\S]*?justify-self: stretch;/
  );
  assert.match(
    homepage,
    /@media \(max-width: 900px\) \{[\s\S]*?\.toc-row\s*\{[\s\S]*?grid-template-columns: 36px 1fr auto;[\s\S]*?\.toc-row \.toc-meta \+ \.toc-meta\s*\{[\s\S]*?display: none;/
  );
});
...
test('shared header progressively compacts without hiding GitHub stars or search', () => {
  ...
  assert.match(headerSource, /target !== 'index\.html' && target !== 'catalog\.html' && target !== 'learning-paths\.html'/);
...
test('learning path query and Enter fallback open the first result predictably', () => {
  assert.equal(
    ...
    '/lessons/13-tools-and-protocols/22-skills-and-agent-sdks/?learningPath=agent-skills'
  );
...
test('career lessons return to their exact learning paths guide without a fake catalog search', () => {
  ...
  assert.match(lessonHtml, /pathReturnLabel = '返回职业路线';/);
...
test('exact Agent Skills search ranks the focused path before individual lessons', () => {
  ...
  try {
    ...
    assert.equal(first.url, '/lessons/13-tools-and-protocols/22-skills-and-agent-sdks/?learningPath=agent-skills');
  } finally {
...
test('lesson reader keeps learning-path context and renders a copyable full-depth install', () => {
  ...
  assert.match(lessonHtml, /var href = lessonHref\(path\);[\s\S]*?'learningPath=' \+ encodeURIComponent\(learningPath\.id\)/);
  assert.doesNotMatch(lessonHtml, /return 'lesson\?path=' \+ encodeURIComponent\(path\) \+[\s\S]{0,100}'&learningPath='/);
  assert.match(lessonHtml, /'第 ' \+ \(focusedIndex \+ 1\) \+ '\/' \+ focusedLessons\.length \+ ' 课'/);
  ...
  assert.match(lessonHtml, /需要先 clone 本仓库/);
  ...
  assert.match(lessonHtml, /在 Claude、Cursor、Codex、OpenClaw、Hermes/);
  assert.match(lessonHtml, /\/check-understanding/);
  assert.match(lessonHtml, /把这节课落实成行动/);
  assert.match(lessonHtml, /if \(article && !article\.querySelector\('#lessonActionPanel'\)\) renderLessonActionPanel\(article\)/);
  ...
  assert.match(lessonHtml, /仓库根目录（包含 README\.md 和 phases\/ 的目录）运行/);
  ...
  assert.match(lessonHtml, /代码围栏不自动等于可运行程序/);
  assert.equal((lessonHtml.match(/var fetchOptions = localPreview \? \{ cache: 'no-store' \} : undefined/g) || []).length, 2);
  assert.match(lessonHtml, /fetch\(rawUrl, fetchOptions\)/);
